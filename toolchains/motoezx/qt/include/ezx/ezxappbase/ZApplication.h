...
#endif
/*!
	functions below are added by wang bin on Aug 23
*/		
	QString getAppChannelName(void);
	void getDefaultStringPath(QString &,QString &);
	int getInputFieldAdapter(void); //return value
	int getKBState();
	QRect getKbInputField(QWidget *);
	...
	virtual void usbStorageInOut(bool);
...
/*!
	functions above are added by wang bin on Aug 23
*/	
...
*/	
	...
	void kbStateChanged(bool,QUuid  const&,int); //?? signal??
	...
	void signalKillactionByMassstorage(void);
	void signalLanguageChange(void);
	...
	void signalTcmdMasterClear(void);
	void signalTcmdMasterReset(void);
	void mainWidgetSizeChanged(int,QRect  const&);


protected slots:
	void animationUpdate(QRect  const&);
	virtual void slotCursorSelected(QWSCursor *);
	virtual void slotHKData(QCString  const&,QArray<char>  const&);
	virtual void slotKbStateChanged(QRect  const&,int,bool,bool);
/*!
	functions above are added by wang bin on Aug 23
*/	

...
# Created with BruceLee's SDK version: #$ get_sdk_version("$SDK_VERSION");
...
all: #$ ExpandGlue("ALL_DEPS",""," "," "); $text .= 'help';

lib: lib-shared
	
lib-shared: $(UICDECLS) $(OBJECTS) $(OBJMOC) #$ Expand("TARGETDEPS");
	$(CC) -shared -o lib$(TARGET).so $(OBJECTS) $(OBJMOC) -lc

lib-static: $(UICDECLS) $(OBJECTS) $(OBJMOC) #$ Expand("TARGETDEPS");
	$(AR) rcs lib$(TARGET).a $(OBJECTS) $(OBJMOC)
	
bin: $(UICDECLS) $(OBJECTS) $(OBJMOC) #$ Expand("TARGETDEPS");
...
dist:
	#$ Substitute('$(TAR) $$PROJECT-src[EZX].tar $$PROJECT.pro Makefile $(SOURCES) $(HEADERS) $(INTERFACES) $(DIST)');
	#$ Substitute('$(GZIP) $$PROJECT-src[EZX].tar');

...
Tips::Tips(const QPoint& pos,const QString& txt,QWidget* p,const char* name,WFlags f)
	:QWidget(p,name,f|WDestructiveClose),ppos(pos),currentText(txt)
{
	//resize(0,0)很重要，否则会在默认位置闪一下
	resize(0,0);
	
	tipBtn=new UTIL_PushButton(QPixmap(close_btn_xpm),QPixmap(close_btn_act_xpm),"",this,0,17,17);
...
void Tips::paintEvent(QPaintEvent* )
{
...
#endif
    {
		...
		r = fontMetrics().boundingRect( 0, 0, sw, 1000, AlignLeft + AlignTop + WordBreak + ExpandTabs, currentText );
    }

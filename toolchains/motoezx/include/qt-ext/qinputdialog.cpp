/****************************************************************************
** $Id: qt/src/dialogs/qinputdialog.cpp   2.3.8   edited 2004-08-05 $
**
...
QInputDialog::QInputDialog( const QString &label, QWidget* parent, const char* name,
    ...
    : UTIL_Dialog(DTSmallSize,true, parent, name, APP_MODAL, 0 )//QDialog( parent, name, modal )
{
	...
	QWidget *content=getDlgContent();
    QVBoxLayout *vbox = new QVBoxLayout( content, 6, 6 );

    QLabel* l = new QLabel( label, content );
    vbox->addWidget( l );

    d->stack = new QWidgetStack( content );
	...
	setDlgCst(buttonBox);
    d->ok = new ZPushButton(NULL, tr( "OK" ), buttonBox );
    //d->ok->setDefault( TRUE );
    ZPushButton *cancel = new ZPushButton(NULL, tr( "Cancel" ), buttonBox );

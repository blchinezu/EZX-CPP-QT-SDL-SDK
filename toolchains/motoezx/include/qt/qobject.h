...
class Q_EXPORT QObject: public Qt  //fox size: $24
{
...
private:
    uint	isSignal   : 1;    //fox f: 4 //1
    uint	isWidget   : 1;               //2
    uint	pendTimer  : 1;               //4  
    uint	pendEvent  : 1;               //8
    uint	blockSig   : 1;               //$10
    uint	wasDeleted : 1;               //$20
    uint	isTree : 1;                   //$40

    QMetaObject *queryMetaObject() const;
    static QMetaObject *metaObj;
    const char	*objname;          //fox f: 8
    QObject	*parentObj;        //fox f: $C
    QObjectList *childObjects;     //fox f: $10
    QSignalDict *connections;      //fox f: $14
    QObjectList *senderObjects;    //fox f: $18
    QObjectList *eventFilters;     //fox f: $1C
    // ### 3.0 todo: add d pointer

    QObject	*sigSender;        //fox f: $20

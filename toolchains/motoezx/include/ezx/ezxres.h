...
{
public:
	QPixmap getIcon(char const* Name, bool);
    QPixmap getIcon(char const* Name, int, int); //fox Add
	static UTIL_Skin *pReader;
	//QImage getImage(QString const&);      //fox remove
	QMovie getMovie(QString const&);
	//QBitmap getBitmap(QString const&);    //fox remove
	RES_ICON_Reader(void);
	~RES_ICON_Reader();
private:

};

#endif  /* !defined(__EZX_RES_H__) */
